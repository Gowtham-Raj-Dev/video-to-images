...
function openHandoffDb() {
    return new Promise((resolve, reject) => {
        ...
        if (!indexedDb) {
            reject(new Error('Current browser does not support local file handoff, please directly open the target debug page and re-select the file.'));
            return;
        }
        ...
        request.onerror = () => reject(request.error || new Error('Unable to open local file handoff.'));
...
export async function saveDebugFileHandoff(file, targetKind = getDebugFileKind(file)) {
    ...
    await new Promise((resolve, reject) => {
        ...
        transaction.onabort = () => reject(transaction.error || new Error('Local file handoff cancelled.'));
    }).finally(() => db.close());
    return record;
}
...
export async function consumeDebugFileHandoff(expectedKind = null) {
    ...
    await new Promise((resolve, reject) => {
        ...
        transaction.onabort = () => reject(transaction.error || new Error('Read local file handoff cancelled.'));
    }).finally(() => db.close());
