...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error('No processable video track in the file');
    }
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error('Video watermark detection confidence is too low, export stopped. You can enable "allow low confidence" and try again.');
    }
    ...
    if (!canEncodeAvc) {
        throw new Error('Current browser does not support WebCodecs H.264/AVC encoding, please use the latest Chrome or Edge.');
    }
